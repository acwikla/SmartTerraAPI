...
void check_device_job_data(){
  ...
  //API wysyla pierwszego DeviceJoba z flaga done=false, nastepnie go usuwa i przysyla kolejnego
  HTTPClient http;
  //ip domowe:192.168.0.164
  http.begin("http://192.168.0.164:5000/api/DeviceJobs/deviceId="+ String(device_id)+"/FalseDoneFlag");
  int httpCode = http.GET();
  Serial.print("httpCode: ");
  Serial.println(httpCode);
  ...
  if (httpCode > 0) //==200
    {
      ...
      const char* job_type = doc["job"]["type"];//obiekt
      const char* job_name = doc["job"]["name"];
      unsigned long jobBodyPump = doc["body"];
      ...
      if((String)job_type=="LED"){
        ...
        if((String)job_name=="TurnOffLED"){
          //doc["body"] body i tak bedzie nullem
          turnoff_LED();
          is_job_done=true;
          set_job_done_property(device_job_id, is_job_done);
        }
