...
#define LAMP_PIN D7
#define PROPELLER_PIN D3
#define WATER_PUMP_PIN D8
#define LED_PIN D6
#define LIQUID_LEVEL_SENSOR D1
#define DHT_PIN D5
...
void get_mode_data(){
  ...
  http.begin("http://"+ String(IP)+"/api/devices/"+ String(device_id)+"/modes");
  //send the request
...
Serial.println(job_type);
      ...
      if((String)job_type=="PUMP" && (String)job_name=="TurnOnWaterPump"){
        check_liquid_level(); 
        Serial.print("is_liquid_level_sufficient: ");
        Serial.println(is_liquid_level_sufficient);
...
void send_terrarium_data(){
  ...
  HTTPClient http;    
  http.begin("http://"+ String(IP)+"/api/Devices/"+ String(device_id)+"/deviceProperties");
...
void send_liquid_level_sensor_data(bool sensor_data){
  ...
  HTTPClient http;
  http.begin("http://"+ String(IP)+"/api/DeviceProperties/"+ String(dev_prop_id)+"/LiquidLevel");
  http.addHeader("Content-Type", "application/json-patch+json");
...
void check_liquid_level(){
  ...
  if(is_liquid_level_sufficient==0){
    ...
    send_liquid_level_sensor_data(is_liquid_level_sufficient);
    Serial.println("Liquid level is not sufficient!");
  }
  else{
...
void turnon_water_pump(unsigned long period, const unsigned long end_task_time, unsigned long start_task_time ){
  ...
  if(is_liquid_level_sufficient==1){
    ...
    {
      Serial.print("start_task_time ");
      Serial.print(start_task_time);
      Serial.print(", end_task_time ");
      Serial.println(end_task_time);
